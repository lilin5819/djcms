...
from .models import Article,Author, Comment, Poll, NewUser
...
def loader(request):
    ...
    loginform = LoginForm()
    context = {'article_detail':'','latest_article_list': latest_article_list, 'loginform':loginform}
    return render(request, 'focus/loader.html', context)
...
def index(request):
    ...
    context = {'author_list':author_list,'article_detail':'','latest_article_list': latest_article_list, 'loginform':loginform}
    return render(request, 'home/index.html', context)

def json_arm(request):
    response = JsonResponse([{'light':'50','cam':'on','temp':'37','hum':'60%'},{'light':'23','cam':'of','temp':'40','hum':'80%'}],safe=False)
    return response

def update(request):
    return render(request,'focus/update.html')
